...
export const uploadImage = async (req, res) => {
  ...
  try {
    ...
    // image params (bucket aws)
    // key: when we upload image we give random gen. name and use the image type
    const params = {
      Bucket: "dojoplugbucket",
      Key: `${nanoid()}.${type}`,
      Body: base64Data,
      ACL: "public-read",
...
export const paidSilver = async (req, res) => {
  try {
    ...
    console.log("SILVER ENROLLMENT ERR", err);
    return res.status(400).send("Silver subscription failed");
  }
};
// gold tier subscribe
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // purchase details
        line_items: [{
            ...
            price: "price_1MmjsoBcG7EIton3pRpX0Cg4",
            // For metered billing, do not pass quantity
            quantity: 1,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      /*line_items: [{
        ...
        line_items: [{
            ...
            price: "price_1MmjtDBcG7EIton32nUVXoo7",
            // For metered billing, do not pass quantity
            quantity: 1,
...
export const paidDiamond = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ["card"],
      // purchase details old single payment
      /*line_items: [{
      ...
      line_items: [{
            ...
            price: "price_1MmjtTBcG7EIton3GerpTCnw",
            // For metered billing, do not pass quantity
            quantity: 1,
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
            ...
            price: "price_1MmjtzBcG7EIton3YJfulS3W",
            // For metered billing, do not pass quantity
            quantity: 1,
...
// bronze  tier success
export const stripeBronzeSuccess = async (req, res) => {
  try {
    // get user from db to get stripe session id
    const user = await User.findById(req.user._id).exec();
    // if no stripe session return
    ...
    // if session payment status is paid, push course to user's course []
    if (session.payment_status === "paid") {
      axios.post("/api/get-account-bronze").then((res) => {
        // console.log(res);
        dispatch({
          type: "LOGIN",
          payload: res.data,
    ...
    console.log("STRIPE BRONZE SUCCESS ERR", err);
    res.json({ success: false });
  }
};
// silver tier success
export const stripeSilverSuccess = async (req, res) => {
  try {
    // get user from db to get stripe session id
    const user = await User.findById(req.user._id).exec();
    // if no stripe session return
    ...
    console.log("SILVER SUCCESS", session);
    // if session payment status is paid, push course to user's course []
    if (session.payment_status === "paid") {
      axios.post("/api/get-account-silver").then((res) => {
        // console.log(res);
        dispatch({
          type: "LOGIN",
          payload: res.data,
    ...
    console.log("STRIPE SILVER SUCCESS ERR", err);
    res.json({ success: false });
  }
};
// gold tier success
export const stripeGoldSuccess = async (req, res) => {
  try {
    // get user from db to get stripe session id
    const user = await User.findById(req.user._id).exec();
    // if no stripe session return
    ...
    console.log("GOLD SUCCESS", session);
    // if session payment status is paid, push course to user's course []
    if (session.payment_status === "paid") {
      axios.post("/api/get-account-gold").then((res) => {
        // console.log(res);
        dispatch({
          type: "LOGIN",
          payload: res.data,
    ...
    console.log("STRIPE GOLD SUCCESS ERR", err);
    res.json({ success: false });
  }
};
// platinum tier success
export const stripePlatinumSuccess = async (req, res) => {
  try {
    // get user from db to get stripe session id
    const user = await User.findById(req.user._id).exec();
    // if no stripe session return
    ...
    console.log("PLATINUM SUCCESS", session);
    // if session payment status is paid, push course to user's course []
    if (session.payment_status === "paid") {
    ...
    console.log("STRIPE PLATINUM SUCCESS ERR", err);
    res.json({ success: false });
  }
};
// silver tier success
export const stripeDiamondSuccess = async (req, res) => {
  try {
    // get user from db to get stripe session id
    const user = await User.findById(req.user._id).exec();
    // if no stripe session return
    ...
    console.log("DIAMOND SUCCESS", session);
    // if session payment status is paid, push course to user's course []
    if (session.payment_status === "paid") {
    ...
    console.log("STRIPE DIAMOND SUCCESS ERR", err);
    res.json({ success: false });
  }
};
// silver tier success
export const stripeStargemSuccess = async (req, res) => {
  try {
    // get user from db to get stripe session id
    const user = await User.findById(req.user._id).exec();
    // if no stripe session return
    ...
    console.log("STARGEM SUCCESS", session);
    // if session payment status is paid, push course to user's course []
    if (session.payment_status === "paid") {
      axios.post("/api/get-account-stargem").then((res) => {
        // console.log(res);
        dispatch({
          type: "LOGIN",
          payload: res.data,
    ...
    console.log("STRIPE STARGEM SUCCESS ERR", err);
    res.json({ success: false });
  }
};
